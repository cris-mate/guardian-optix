  ...
  for (let dayOffset = -3; dayOffset <= 7; dayOffset++) {
    ...
    if (dayOffset !== 0 || Math.random() > 0.3) {
      ...
      shifts.push({
        ...
        shiftType: 'Morning',
        tasks: [
          { _id: 't1', description: 'Patrol perimeter', frequency: 'hourly', completed: dayOffset < 0 },
          { _id: 't2', description: 'Check all access points', frequency: 'once', completed: dayOffset < 0 },
          { _id: 't3', description: 'Log visitor entries', frequency: 'periodic', completed: false },
        ],
    ...
    if (Math.random() > 0.4) {
      ...
      shifts.push({
        ...
        shiftType: 'Afternoon',
        tasks: [
          { _id: 't4', description: 'Monitor CCTV feeds', frequency: 'periodic', completed: dayOffset < 0 },
          { _id: 't5', description: 'Secure premises at shift end', frequency: 'once', completed: dayOffset < 0 },
        ],
    ...
    if (Math.random() > 0.5) {
      ...
      shifts.push({
        ...
        shiftType: 'Night',
        tasks: [
          { _id: 't6', description: 'Hourly patrol rounds', frequency: 'hourly', completed: false },
          { _id: 't7', description: 'Check fire exits', frequency: 'once', completed: false },
          { _id: 't8', description: 'Incident reporting', frequency: 'periodic', completed: false },
        ],
        status: dayOffset < 0 ? 'completed' : 'scheduled',
        createdAt: new Date().toISOString(),
      });
